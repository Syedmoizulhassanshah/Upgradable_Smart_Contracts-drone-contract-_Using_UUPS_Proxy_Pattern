...
7. We now change proxyAddress in **upgrade_UUPS.js** with the new **DroneContractProxy address** present in the code line `const proxyAddress = '0x1397cbC428E0Bfc599eFB31640F909C12144CDB5';`
8. Once the **Drone_Contract_UUPS** is deployed and verified on rinkeby testnet, we now need to deploy the upgraded implementation of Drone-Contract that
...
9. Now verify the deployed  **Drone_Contract_UUPS** on rinkeby testnet using the following command:
   `npx hardhat verify --network rinkeby {implementation  contract-address of Drone_ContractV2_UUPS}`
10. Once **Drone_ContractV2_UUPS** verified on Etherscan, then perform the same procedure mentioned in the **step:6**.
